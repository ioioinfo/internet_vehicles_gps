	...
	server.route([
        //获得所有车辆
        {
            ...
            handler: function(request, reply) {
				...
				server.plugins['models'].gps_history.account_history_records(info,function(err,rows){
                    if (!err) {
        ...
        {
            method: 'POST',
            path: '/save_history_record',
            handler: function(request, reply){
                var record = request.payload.history_record;
                ...
                // var record ={
                //     "gps_id":1,
                //     "longitude":"121.439943",
                //     "latitude":"31.343803",
                //     "time":"2017-09-12 14:53:00"
                // }

                server.plugins['models'].gps_history.save_history_record(record, function(err,result){
        ...
        //删除
        {
            ...
            handler: function(request, reply){
                ...
                server.plugins['models'].gps_history.delete_history_record(id, function(err,result){
                    if (result.affectedRows>0) {
                        return reply({"success":true,"service_info":service_info});
                    }else {
                        return reply({"success":false,"message":result.message,"service_info":service_info});
        ...
        //查询渠道
        {
            ...
            handler: function(request, reply) {
                ...
                //查询渠道部门
                server.plugins['models'].gps_history.search_history_byId(id,function(err,rows){
                    if (!err) {
                        ep.emit("rows", rows);
                    }else {
                        ep.emit("rows", []);
